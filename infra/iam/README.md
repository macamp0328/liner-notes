...
> **Heads-up for existing deployments:** the read-only `iam:GetOpenIDConnectProvider` grant on `operator-iam-policy.json` was added once the GitHub OIDC provider ([#209](https://github.com/macamp0328/liner-notes/pull/209)) entered Terraform state. If your `liner-notes-iam` managed policy predates it, push a new version (see [Updating a managed policy](#updating-a-managed-policy)) — otherwise the operator's day-to-day `terraform apply` fails on **refresh** of the provider with `AccessDenied`.
