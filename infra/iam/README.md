...
> **Heads-up for existing deployments:** the read-only `ReadGraphBackups` grant on `operator-deploy-policy.json` was added with the weekly S3 graph backup ([#508](https://github.com/macamp0328/liner-notes/issues/508), backup itself shipped in [#104](https://github.com/macamp0328/liner-notes/issues/104)). If your `liner-notes-deploy` managed policy predates it, push a new version (see [Updating a managed policy](#updating-a-managed-policy)) — otherwise the restore flow's `aws s3 ls`/`aws s3 cp` (see the [runbook's "Graph backup and restore"](../RUNBOOK.md#graph-backup-and-restore)) fails with `AccessDenied` under the operator profile and needs the admin `root` profile instead.
