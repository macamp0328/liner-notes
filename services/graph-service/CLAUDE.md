...
`track-recording-artists` (#335, pushes MB recording performance credits down to track-scoped
`CREDITED_ON` edges, resolving each performer by the `musicbrainzId` join),
...
10. **MB recording performance credits are pushed down to track-scoped `CREDITED_ON` by a deterministic MBID join (#335).** The `track-recording-artists` enrichment fetches `/recording/{mbid}?inc=artist-rels` for each `recordingMbid` and writes the **performance** relations (`performer`/`instrument`/`vocal`) as `(:Musician)-[:CREDITED_ON {scope:"track", source:"musicbrainz", recordingMbid}]->(:Track)`. Production/engineering roles (`producer`/`mix`/`recording`/`mastering`/…) stay release-scoped and are **never** pushed down (the client filters them). Each performer is resolved by `MERGE (m:Musician {musicbrainzId})` — the same MB-artist MBID the `mb-artist-id` pass (#380) already stored — so the stage **deps `mb-artist-id`** (running it first lets a performer whose Discogs↔MB link resolved be reused instead of duplicated). A performer that **cannot** be resolved this way — one MusicBrainz has no Discogs link for, so no in-collection node carries that MBID — gets an **MBID-keyed fallback `Musician`** (`musicbrainzId` set, no `discogsId`) linked to any same-MBID `Artist` via `SAME_PERSON_AS`, never merged by name. That fallback can still be a second node for a person who also exists only as a `discogsId`-keyed `Musician` without a `musicbrainzId`; this is the deliberate clean-data tradeoff (provenance preserved over a fuzzy name-merge), not a bug. The credit props use `ON CREATE SET`, so an existing Discogs credit for the same person/track is **never clobbered** — MB only fills gaps. Reuses `parseDisplayRole`/`parseRoleCategory`/`parseInstrument` so MB credits are queryable via `/explore/instrument/:name`. Coverage is `/stats.tracksWithMbRecordingArtists` (verify-gated for silently-zero, denominator = tracks with a `recordingMbid`); `/track-recording-artists/reset` deletes the MB credits + the fallback nodes.
