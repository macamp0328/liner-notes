...
All `/explore/*` routes return a **bare JSON array** — _not_ a `{ data }` envelope — except two:
`/explore/connections/:discogsId` returns `{ seed, nodes }`, and `/explore/instrument/:name` returns
`{ credits, players }` (the two instrument axes, #393).
...
**17** pipelines — `lyrics`, `nationality`, `master-data`, `mb-release-events`, `track-musicbrainz`,
...
Work `writerMbids` to `WROTE` edges by the `musicbrainzId` join) — and for each:
...
- `POST /api/v1/admin/<stage>/reset` — force a full re-fetch. **Exists for 12 stages only** — the
  five _without_ a `reset` route are `lyrics` (use `/api/v1/admin/lyrics/clear-genius` instead),
  `master-data`, `artist-genres` (a self-idempotent whole-graph aggregation with nothing to reset),
  `person-reconciliation` and `songwriter-reconciliation` (both re-link exhaustively every run —
  nothing to reset). `label-hierarchy` and `group-members` both _have_ a reset (label-hierarchy
...
| `stats`                                                 | `{ data }`              |
| `explore/*` (except `connections`, `instrument`)        | **bare array**          |
