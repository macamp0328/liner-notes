...
| `Master`      | `discogsId` (unique), `title`, `year`, `mbReleaseEventsFetchedAt` (datetime), `musicbrainzReleaseGroupId` (nullable — the MB release-group MBID resolved by the `mb-release-events` pass, persisted as a provenance/crosswalk attribute per ADR 0005 law 5, #444) — the canonical album grouping a `Release` is a pressing of. Holds the original-year + global pressing-country/format facts (see `RELEASED_IN` / `MB_RELEASED_IN`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `MB_RELEASED_IN` | Master → Country             | `mbReleaseId` (merge key), `date`, `formats`, `source` (`"musicbrainz"`, #442) — release events from the MusicBrainz enrichment                                                                                                                                                                                                                                                                                                                                                     |
...
All `/explore/*` routes return a **bare JSON array** — _not_ a `{ data }` envelope — except four:
...
`{ influencedBy, influenced }` (the two influence directions, #391), and `/explore/path` returns
`{ from, to, found, length, maxDepth, steps }` (the shortest human/credit path, #343).
...
from MB recording `place-rels`, MERGEing the Studio by the canonical `nameKey` (#443) + capturing Place coordinates/area),
...
| `stats`                                                 | `{ data }`                                     |
| `explore/*` (except the four below)                     | **bare array**                                 |
