...
| `Recording`   | `mbid` (unique), `title` — a **fallback link target** for recording↔recording derivative lineage (#434), keyed on the MusicBrainz recording MBID. Created only by the `track-recording-lineage` pass as the head of a `RELATED_RECORDING` edge; captures out-of-collection originals/derivatives. An in-collection counterpart is **not** a `Recording` node — it is a `Track` with `recordingMbid`, resolved at query time via the `Recording.mbid → Track.recordingMbid` join. Sparse by design.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `RELATED_RECORDING`     | Track → Recording            | `source` (`"musicbrainz"`), `type` (raw MB derivative type — `remix`/`DJ-mix`/`edit`/`mashes up`/`a cappella`/`instrumental`/`karaoke`/`compilation`, part of the MERGE key), `direction` (raw MB `forward`/`backward`), `recordingMbid` (source recording, provenance) — recording↔recording derivative lineage (#434) from MB recording `recording-rels`, fanned out to every Track sharing the source `recordingMbid` by the `track-recording-lineage` pass. `type`/`direction` stored RAW (never normalised → never backwards). Sparse by design. Backs `GET /explore/lineage/:mbid`. |
...
| `GET`  | `/api/v1/explore/lineage/:mbid`             | Recording↔recording derivative lineage for a MusicBrainz recording (#434): each `RELATED_RECORDING` link (remix/edit/instrumental/…) with raw `type` + `direction`, a human `phrase`, and the in-collection releases carrying the related recording (empty = out-of-collection). Bare array; sparse by design                                                                                                           |
...
**21** pipelines — `lyrics`, `nationality`, `master-data`, `mb-release-events`, `track-musicbrainz`,
`track-works` (#336, writes `RECORDING_OF` to MBID-keyed `Work` nodes from MB recording→work rels),
`track-recording-artists` (#335/#339/#434, pushes MB recording performance, production
(producer/engineer), arranging (arranger family, `composer`-bucketed) **and** remixer (`other`-bucketed,
#434) credits down to track-scoped `CREDITED_ON` edges, resolving each person by the `musicbrainzId` join),
...
`track-recording-lineage` (#434, writes track-scoped `(:Track)-[:RELATED_RECORDING {source:"musicbrainz", type, direction}]->(:Recording)`
from MB recording `recording-rels` to MBID-keyed fallback `Recording` nodes; reset prunes orphan
`Recording` nodes), `track-acousticbrainz`, `track-deezer`, `artist-profiles`, `artist-genres`, `artist-wikidata` (#341/#393,
...
* `POST /api/v1/admin/<stage>/reset` — force a full re-fetch. **Exists for 14 stages only** — the
