...
- **Two follow-ups, one now done.** `track-acousticbrainz` — the strongest terminal case but an
  ...
  transient blip would have permanently exhausted every in-flight track. `mb-release-events` remains
  medium-confidence and would need its no-op-warning re-audited; it is still filed separately.
