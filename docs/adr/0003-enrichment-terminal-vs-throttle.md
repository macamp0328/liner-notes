...
| `mb-release-events`    | Throttle-only (follow-up)      | MusicBrainz _can_ add a release-group relation later — medium confidence. Converging it means re-auditing its no-op warning and `mastersSkipped` semantics; deferred.                                            |
| `track-acousticbrainz` | Throttle-only here (follow-up) | The **strongest** terminal case — AcousticBrainz is frozen/read-only (reconfirmed by #377), so a genuine 404 for an MBID is permanent. But it is an off-contract **batch** stage with no `resolve`; see below.   |
...
- **Two follow-ups deferred.** `track-acousticbrainz` is the strongest terminal case but is an
  off-contract batch stage (no `resolve`); its terminal concept generalizes via a batch-local
  `acousticBrainzExhausted` boolean + `IS NULL` gate written _outside_ the runner contract, and it
  shifts batch `*Skipped` counting semantics — its own reviewable unit. `mb-release-events` is
  medium-confidence and would need its no-op-warning re-audited. Both are filed separately.
