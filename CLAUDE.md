...
# scripts:test. (yamllint/tflint/trivy stay CI-only — external Python/binary tools.)
pnpm verify

# Doc/config/Terraform lint gates (CI re-runs all of these — see CI Requirements).
# markdownlint is a devDependency (also part of `pnpm verify`); yamllint/tflint/trivy
# are CI-pinned (no package.json entry) with the local fallbacks shown.
...
# trivy (Terraform security posture — one invocation covers both roots; intentional
# findings are suppressed inline via `#trivy:ignore:` + reason): install trivy, then
#   trivy config --exit-code 1 --skip-check-update --misconfig-scanners terraform infra/terraform
...
| CI (`ci.yml` & co.) | 17-job fan-out (incl. `script-tests`, `terraform`, shellcheck, `hadolint`, `kubeconform`, `markdownlint`, `yamllint`, `tflint`, `trivy`) + drift guards (diagrams, insomnia/openapi) | everything above, re-checked server-side; secrets (TruffleHog), CVEs (audit), CodeQL findings; terraform fmt/validate + lint (tflint/AWS) + security posture (trivy); `*.sh` lint; Dockerfile lint (hadolint); k8s manifest schemas (kubeconform); Markdown structure (markdownlint) + YAML semantics (yamllint) |
...
| Terraform security | trivy config (pinned binary, embedded checks)          | Both roots clean at every severity; intentional findings suppressed inline with `#trivy:ignore:` + a reason comment |
...
| Security audit     | `pnpm audit` via pnpm 11 (audit job only — see note)   | No high/critical vulnerabilities                                                                                    |
...
> **Doc/config/terraform lint (#348, #349):** `markdownlint` and `yamllint` cover the semantic/structural tail Prettier doesn't — they deliberately enable **only** high-signal rules (Markdown heading hierarchy + in-repo links; YAML duplicate-key + ambiguous truthy) and disable everything Prettier formats, so the linters never fight the formatter. `tflint` (+ AWS ruleset) adds Terraform anti-pattern/misconfig linting on top of `fmt`/`validate`. All of these **hard-fail** (show red on the PR) like the sibling quality jobs (`terraform`, `hadolint`, `kubeconform`); which checks actually _block_ merge is curated in the branch-protection ruleset, not by the job existing. **Dead-link checking was deliberately skipped** — a network-dependent link checker (e.g. lychee) would make PR CI flaky for little gain, and markdownlint's MD051 already catches broken _in-repo_ anchors.

> **Terraform security posture (#395):** `trivy config` (the maintained tfsec successor) scans both terraform roots for security misconfigurations — the deferred second half of #348 that tflint doesn't cover (open security groups, unencrypted resources, public exposure). It runs with the checks **embedded in the pinned binary** (`--skip-check-update` — deterministic, no runtime bundle fetch) and hard-fails at **every severity**: the curated baseline is zero findings, with every known-intentional, cost-driven choice (SSE-S3 not KMS, no VPC flow logs, open egress, public subnet, etc.) suppressed **inline** in the `.tf` file via `#trivy:ignore:AVD-AWS-XXXX` directly above the resource, always with a reason comment. New findings = red job; suppress them where the resource lives (with rationale), never via CI flags or severity floors. Like its siblings the job is red-on-PR but non-merge-blocking until promoted in the branch-protection ruleset.

> **Security-audit pnpm exception (#502):** the `audit` CI job runs **pnpm 11** — the only place in the repo that does. npm retired the legacy audit endpoint (410, July 2026) and pnpm 9/10 call it; pnpm 11 uses the replacement bulk advisory endpoint. The job has **no install step** (`pnpm audit` reads only `pnpm-lock.yaml`), so pnpm 11 never touches `node_modules` or the lockfile — the install/lockfile toolchain stays **pnpm 9.15.9** everywhere (the `.mise.toml` pin; pnpm 10/11 must never run `install` or write the lockfile — they strip `libc` fields and ignore `pnpm.overrides`). The `pnpm.overrides … ignored` warning in the job log is expected and harmless: the lockfile is already resolved with those overrides by pnpm 9.
