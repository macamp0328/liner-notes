...
2. **Evaluate the suggestion** against:
   ...
   - Does it align with the conventions in `CLAUDE.md` (strict TypeScript, ESM/NodeNext, pnpm workspaces, no over-engineering)?
   - Is the project's deliberate design (graph-service as sole Neo4j access layer, Fastify plugin architecture) being respected?
...
3. **Decide and act:**
   ...
   **If declining:**
   ...
   - Examples: "intentional per CLAUDE.md — graph-service is the single Neo4j access layer", "ESM NodeNext requires `.js` extensions on relative imports by design", "this abstraction is load-bearing for the ingestion pipeline"
...
- Fix: "Fixed — extracted the constant as suggested."
- Decline: "Keeping this as-is — the `.js` extension on relative imports is required by ESM NodeNext per CLAUDE.md."
- Partial: "Updated the return type annotation. Left the structure unchanged — the repository pattern here is intentional per the graph-service design."
