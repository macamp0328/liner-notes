...
# The `(^|[^[:alnum:]_])git[^|;&]*(commit|push)` shape tolerates global options BETWEEN
# `git` and the subcommand (e.g. `git -c core.hooksPath=/dev/null commit --no-verify`,
# `git -C path push --force`) so they can't slip past, while the leading non-identifier
# boundary keeps `legit commit`/`digit push` from matching, and the `[^|;&]` class still
# stops a `;`/`|`/`&&`-chained later command from being glued onto the `git` invocation.
...
if printf '%s\n' "$normalized" | /usr/bin/grep -E '(^|[^[:alnum:]_])git[^|;&]*(commit|push)' | /usr/bin/grep -q -- '--no-verify'; then
...
if printf '%s\n' "$normalized" | /usr/bin/grep -Eq -- '(^|[^[:alnum:]_])git[^|;&]*push[^|;&]*( -f([ "'\'']|$)|--force([ "'\'']|$))'; then
